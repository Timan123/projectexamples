...
//		->select('GlobalLogoID')->distinct()->get();
		...
		$customers = DB::table('BillingSystem.dbo.IPv4Billing')
				...
				->where('id',7872)
				->select(['GlobalLogoID','id'])
				->get();
		
//		$customers = [['id'=> 5793, 'GlobalLogoID' => 'G1273OVG']];
		...
		foreach($customers as $customer) {
			...
			$orders = DB::table('dbo.v_sdscorecard as sd')
				...
				->whereNotIn('ProvOrderStatus',['Cancelled','New'])
			...
			foreach($orders as $order) {
				...
				$orderArr = ['OrderId' => $orderId, 
							...
							'BillStartDt' => $order['BillStartDt'], 'Term' => $order['TermDesc'], 'AutoRenew' => $order['AutoRenew'], 'Investigate' => 1];
				...
				foreach($ips as $ip) {
					$numIps++;
					$totalIps += $ip['count'];
					array_push($addArr,['GlobalLogoID' => $customer, 'OrderId' => $orderId, 'Address' => $ip['Address'], 'Investigate' => 1]);
				}
				if ($totalIps > 0) {
