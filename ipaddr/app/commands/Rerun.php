...
class Rerun extends Command
{
	...
	{ 
		...
		$customers = Config::get('rerun.reRunSet');
...
//		->select('GlobalLogoID')->distinct()->get();
		...
		foreach($customers as $customer) {
			...
			foreach($orders as $order) {
				...
				if ($order['InterfaceTypeInt'] == 40) {
					$mult = DB::table('dbo.v_sdscorecard as sd')
							->leftJoin('mjain.OPM_Order_Details as ood','sd.OrderId','=','ood.OrderId')
							->where('ParentOrderId',$orderId)->whereIn('InterfaceTypeInt',[6,12,10,8,13, 14, 33, 42, 45])->count();
				}
			...
			if ($customerTotal > 2) {
				...
				DB::table('BillingSystem.dbo.IPv4BillingRerun')
					->insert(
				...
				DB::table('BillingSystem.dbo.IPv4BillingToAddressesRerun')
					->insert(
					$addArr);
				DB::table('BillingSystem.dbo.IPv4BillingToOldOrderRerun')
					->insert(
					$orderArrMulti);
				echo "$customer\n"; 
			}
		...
		//wrap up stuff
		DB::table('BillingSystem.dbo.IPv4BillingRerun')->where('IPCount','>',8000)->update(['JumboNoBill' => 1]);
