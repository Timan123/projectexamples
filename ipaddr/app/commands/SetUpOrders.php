 ...
 * @author tcassidy
 */
class SetUpOrders extends Command
{
	/**
	...
	{ 
		...
		$ordersToSetUp = DB::table('BillingSystem.dbo.IPv4Billing')
				->where('id',7872)				
...
//		return;
		...
		foreach($ordersToSetUp as $orderToSetup) {
			...
			unset($arr['F2Close']);
			unset($arr['OrderType']);
			$arr['Comments'] = "Auto IPv4 Order for $ipCount IP's";

			//echo var_dump($arr);
			DB::table('BillingSystem.dbo.IPv4Billing')->where('id',$id)->update(['SetupStep' => 'Table_V']);
			...
			DB::table('BillingSystem.dbo.IPv4Billing')->where('id',$id)->update(['SetupStep' => 'OPM']);
			...
			if (!$lineInsert) {
				DB::table('BillingSystem.dbo.IPv4Billing')->where('id',$id)->update(['SetupStep' => 'OPMLine']);
				...
				DB::table('BillingSystem.dbo.IPv4Billing')->where('id',$id)->update(['LineInsert' => 1]);
			}

			DB::table('BillingSystem.dbo.IPv4Billing')->where('id',$id)->update(['SetupStep' => 'OrderSch']);
			$sch = ['SLA'=> $billDt,
					'AssignedDt'=> $billDt,
					'IPCompDt'=> $billDt,
			...
			DB::table('BillingSystem.dbo.IPv4Billing')->where('id',$id)->update(['SetupStep' => 'Prov']);
			...
			//CCDB SetupStep
			if (!$ccdbInsert) {
				DB::table('BillingSystem.dbo.IPv4Billing')->where('id',$id)->update(['SetupStep' => 'CCDB']);
				...
				DB::table('BillingSystem.dbo.IPv4Billing')->where('id',$id)->update(['CCDBInsert' => 1]);
			}

			DB::table('BillingSystem.dbo.IPv4Billing')->where('id',$id)->update(['SetupStep' => 'Done']);
